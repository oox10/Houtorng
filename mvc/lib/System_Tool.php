  ...
  **************************************************/
  ...
  class System_Tool{
	...
	*******************************************************************/
	...
	public static function log_write($Target_logs,$User,$FromAP,$Logs_String){
    
	  $LogStringHeader = date('Y-m-d h:i:s')." -[ ".System_Tool::get_client_ip()." ] :\t";
	  $LogStringBody   = $User."\t@ ".$FromAP."\t:= ";
	  switch($Target_logs){  
	    
		case 'Systemlogin':
		  $Logs_String = $LogStringHeader.$Logs_String;
		  break;
		  
		case 'SearchSys':
		  $Logs_String = $LogStringHeader.$LogStringBody.$Logs_String;
		  break;
	  ...
	  $handle = fopen(_SYSTEM_ROOT_PATH."Logs/SearchSys.log", "a");
      fwrite($handle, $Logs_String."\n");
	  fclose($handle);
