  ...
  class Report_Model extends Admin_Model{
	...
	public function __construct(){
	  ...
	  if(isset($_SESSION[_SYSTEM_NAME_SHORT]['USER'])){
		$this->USER = unserialize($_SESSION[_SYSTEM_NAME_SHORT]['USER']);  
	  }
	}
	...
	// R00 : All Record  /   R01 : Group Record  /  OTHER: SELF Report
	public function ADReport_Get_Report_List(){
	  
	  $result_key = parent::Initial_Result('reports');
	  ...
	  try{
		...
		if( in_array('R00',$this->USER->PermissionNow['group_roles']) ){  
		  //1. R00 可取得所有回報資料
		  $DB_OBJ = $this->DBLink->prepare(  SQL_AdReport::ADMIN_REPORT_SELECT_SYSTEM_REPORTS() );	
		  $result['mode']   =  'admin';
		
		}else if(in_array('R01',$this->USER->PermissionNow['group_roles'])){ 
		  // 2. R01 可取得群組回報資料
		  $DB_OBJ = $this->DBLink->prepare(parent::SQL_Permission_Filter(SQL_AdReport::ADMIN_REPORT_SELECT_GROUP_REPORTS()));	
		  $DB_OBJ->bindValue(':group_code', $this->USER->PermissionNow['group_code'] );
		}else{
		  //3. 其他可取得自己的回報資料
		  $DB_OBJ = $this->DBLink->prepare(parent::SQL_Permission_Filter(SQL_AdReport::ADMIN_REPORT_SELECT_USER_REPORTS())); 
		...
		$result['action'] = true;		
		$result['data']   = $reports;
		$result['session']['reports']= $report_pool;
	...
	public function ADReport_Get_Report_Data($ReportQueue=array() , $DataNo=0){
	  ...
	  try{  
		...
		// 取得資料
		$report_data = NULL;
		$DB_GET	= $this->DBLink->prepare( SQL_AdReport::ADMIN_REPORT_GET_REPORT_DATA() );
	...
	public function ADReport_Set_Report_Note($ReportQueue=array() , $DataNo=0 , $UMessage=''){
	  ...
	  try{  
		...
		// 更新訊息
		$report_data = NULL;
		$DB_GET	= $this->DBLink->prepare( SQL_AdReport::ADMIN_REPORT_SET_REPORT_NOTE() );
		$DB_GET->bindParam(':fno'   , $DataNo , PDO::PARAM_INT);
	...
	public function ADReport_Update_Report_Field($ReportQueue=array() , $DataNo=0 , $FieldUpdate=''){
	  ...
	  try{  
		...
		// 更新訊息
		$report_data = NULL;
		$DB_SET	= $this->DBLink->prepare( SQL_AdReport::ADMIN_REPORT_UPDATE_REPORT_DATA(array_keys($FieldUpdate)));
	...
	public function ADReport_User_Feedback_Submit($RpFrom='' , $RpData=array()  ){
	  ...
	  try{  
		...
		// 確定存取權限	
		$DB_OBJ = $this->DBLink->prepare(SQL_AdReport::ADMIN_REPORT_USER_FEEDBACK_INITIAL());
	
		$DB_OBJ->bindValue(':fb_from' 		, $RpFrom );
		$DB_OBJ->bindValue(':fb_group' 		, 'iis' );  // 管理系統回報預設為 群組1
