<?php
  ...
  /***
  *   TLCDA Admin System Account & Login SQL SET
  *  
  *
  */
  ...
  class SQL_Account{
	...
	public static function CHECK_ACCOUNT_REGIST_EMAIL(){
	  $SQL_String = "SELECT uno,user_id,user_mail,user_status FROM user_info LEFT JOIN user_login ON uid=uno WHERE user_mail=:user_mail AND uno IS NOT NULL;";
	  return $SQL_String;
	}
	...
	public static function GET_ACCOUNT_GROUPS(){
	  $SQL_String = "SELECT gid,master,ug_no,ug_name,ug_info,ug_pri,COLUMN_JSON(rset) AS roles FROM permission_matrix LEFT JOIN user_group ON ug_code=gid WHERE uid=:uid;";
	  return $SQL_String;
	}
	
	//-- 取得使用者群組資源篩選條件
	public static function GET_GROUPS_ACCESS_RULES(){
	  $SQL_String = "SELECT * FROM permission_rule WHERE mode='acl' AND limitto='group' AND target=:gid AND _keep=1;";
	  return $SQL_String;
	}
	
	//-- 取得使用者群組角色動作
	public static function GET_GROUPS_ROLE_ACTION($roles=array()){
	  $role_select = join("','",$roles);
	  $SQL_String = "SELECT * FROM permission_rule WHERE mode='rbac' AND _keep=1 AND limitto='role' AND target IN ('".$role_select."');";
	  return $SQL_String;
	}
	...
	public static function SELECT_REGIST_GROUP_LIST(){
	  $SQL_String = "SELECT ug_code,ug_name,ug_info FROM user_group WHERE ug_pri < 8 AND ug_pri > 0 ORDER BY ug_pri DESC , ug_info ASC , ug_no ASC;";
	  return $SQL_String;
	}
	...
	public static function INSERT_NEW_USER_INFOMATION(){
	  $SQL_String = "INSERT INTO user_info VALUES(:uid,:user_name,:user_idno,:user_staff,:user_organ,:user_tel,:user_mail,:user_address,'','',:user_pri);";
	  return $SQL_String;
	}
	...
	public static function INSERT_GROUP_MEMBER(){
	  $SQL_String = "INSERT INTO permission_matrix VALUES(:uno,:gno,COLUMN_CREATE('R00', 0, 'R01', 0, 'R02', 0, 'R03',0, 'R04',0, 'R05',1),:master,:creater,NULL);";
	  return $SQL_String;
	}
	...
	public static function ACCOUNT_START_AND_SET_PASSWORD(){
	  $SQL_String = "UPDATE user_login LEFT JOIN user_info ON uno=user_info.uid LEFT JOIN user_regist ON uno = user_regist.uid SET user_pw=:user_pass,user_status=:user_status,reg_state=:reg_state,user_pri=3 WHERE uno=:uno AND reg_code=:reg_code;";
	  return $SQL_String;
	}
