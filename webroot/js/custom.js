...
jQuery(document).ready(function ($) {
	...
	if($(".sublv-switch").length){
	  $(".sublv-switch").click(function(){
		$(this).parent().children('ul.menu').toggle();  
	  });
	}
    ...
    if (contactForm && contactForm.length > 0) {
        ...
        contactFormSubmit.bind("click", function (evt) {
            ...
            if(!$('#captcha_input').html().length){
			  alert("Please keyin the CAPTCHA");
			  $('#captcha_input').focus();
			  return false;
			}  
            ...
            if (contactForm.valid()) {
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "index.php",
                    data: 'act=message&captcha='+$('#captcha_input').html()+'&message='+getFormData(),
					dataType:'json',
                    statusCode: {
			...
			function resetCaptcha(){
			  contactForm.find('#captcha_input').html('');
